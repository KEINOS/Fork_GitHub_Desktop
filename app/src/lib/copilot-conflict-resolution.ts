...
/**
 * Extracts a lowercased, searchable haystack from an unknown error so we can
 * classify it without depending on the concrete error type. Includes the
 * message, name, and any string-coercible `code`/`status` fields commonly set
 * by transport and HTTP errors.
 */
function describeError(error: unknown): string {
  if (error instanceof Error) {
    ...
    if (maybe.status !== undefined) {
      parts.push(String(maybe.status))
    }
...
/**
 * Maps an error thrown during Copilot conflict resolution to a concise,
 * actionable message for the user.
 *
 * The lower layers preserve the distinction between {@link CopilotValidationError}
 * (bad model output) and transport failures (auth, network, rate limiting,
 * timeouts). This surfaces that distinction so the dialog can tell the user
 * whether to sign in again, retry later, or fall back to manual resolution —
 * rather than collapsing everything into one opaque message.
 */
export function getCopilotConflictResolutionErrorMessage(
  error: unknown
): string {
  if (error instanceof CopilotValidationError) {
    return "Copilot couldn't resolve these conflicts. Try again, or resolve them manually."
  }

  const haystack = describeError(error)

  if (haystack.includes('timed out') || haystack.includes('timeout')) {
    return 'Copilot timed out while resolving these conflicts. Try again, or resolve them manually.'
  }
  ...
  if (
    ...
    return "You've hit Copilot's rate limit. Wait a moment and try again."
  }

  if (
    haystack.includes('401') ||
    haystack.includes('403') ||
    haystack.includes('unauthor') ||
    haystack.includes('authentication') ||
    haystack.includes('no token') ||
    haystack.includes('token expired') ||
    haystack.includes('sign in')
  ) {
    return 'Your Copilot session has expired. Sign in to Copilot again and retry.'
  }
  ...
  if (
    ...
    return "Couldn't reach Copilot. Check your internet connection and try again."
  }
